...
@app.route('/')
def index():
    confFile = open(os.getcwd() + "/yggnode-api/config/annexes.yml", 'r')
...
def generatingTorrent():
    remoteTempTorrent()
    if not (os.path.isfile(os.getcwd() + "/yggnode-api/blackhole/torrents/" + request.args.get("id") + ".torrent")):
        return "torrent unavailable"
    # grab torrent file matching id provided
    my_torrent = Torrent.from_file(os.getcwd() + "/yggnode-api/blackhole/torrents/" + request.args.get("id") + ".torrent")
    ...
    my_torrent.to_file(os.getcwd() + "/yggnode-api/blackhole/torrents/tmp/" + request.args.get("id") + request.args.get("passkey") + ".torrent")
    # send torrent file
    return send_file(os.getcwd() + "/yggnode-api/blackhole/torrents/tmp/" + request.args.get("id") + request.args.get("passkey") + ".torrent",
                     as_attachment=True,
...
def generatingRSS():
    ...
    if not (os.path.isfile(os.getcwd() + "/yggnode-api/blackhole/rss/" + request.args.get("id") + ".xml")):
    ...
    rssFile = open(os.getcwd() + "/yggnode-api/blackhole/rss/" + request.args.get("id") + ".xml", "r")
    txt = rssFile.read()
    rssFile.close()
...
def remoteTempTorrent():
    ...
    for torrentFile in os.listdir(os.getcwd() + "/yggnode-api/blackhole/torrents/tmp/"):
        if os.stat(os.getcwd() + "/yggnode-api/blackhole/torrents/tmp/" + torrentFile).st_mtime < now:
            os.remove(os.getcwd() + "/yggnode-api/blackhole/torrents/tmp/" + torrentFile)
...
def getStatus():
    ...
    renderTxt = ""
    for index in range(len(serverConfiguration["Categories"]["id"])):
        renderTxt += "<strong>" + serverConfiguration["Categories"]["idLabel"][index] + "</strong> : " + str(time.ctime(os.stat(os.getcwd() + "/yggnode-api/blackhole/rss/" + str(serverConfiguration["Categories"]["id"][index]) + ".xml").st_mtime)) + "<br>"
    renderTxt += "<br><br>"
    for index in range(len(serverConfiguration["sub-Categories"]["id"])):
        renderTxt += "<strong>" + serverConfiguration["sub-Categories"]["idLabel"][index] + "</strong> : " + str(time.ctime(os.stat(os.getcwd() + "/yggnode-api/blackhole/rss/" + str(serverConfiguration["sub-Categories"]["id"][index]) + ".xml").st_mtime)) + "<br>"

    return renderTxt

if __name__ == '__main__':
    # initialize working environment for python server
    if not (os.path.exists(os.getcwd() + "/yggnode-api/blackhole/rss/")):
        os.mkdir(os.getcwd() + '/yggnode-api/blackhole/rss')
    if not (os.path.exists(os.getcwd() + "/yggnode-api/blackhole/torrents/")):
        os.mkdir(os.getcwd() + '/yggnode-api/blackhole/torrents')
    if not (os.path.exists(os.getcwd() + "/yggnode-api/blackhole/torrents/tmp")):
        os.mkdir(os.getcwd() + '/yggnode-api/blackhole/torrents/tmp')
