...
def generatingTorrent():
    remoteTempTorrent()
    if not os.path.exists(f'blackhole/torrents/{request.args.get("id")}.torrent'):
        return "torrent unavailable"
    my_torrent = Torrent.from_file(f'blackhole/torrents/{request.args.get("id")}.torrent')
    ...
    my_torrent.to_file(f'blackhole/torrents/tmp/{request.args.get("id")}.{request.args.get("passkey")}.torrent')
    # send torrent file
    return send_file(f'blackhole/torrents/tmp/{request.args.get("id")}.{request.args.get("passkey")}.torrent',
                     as_attachment=True,
...
def generatingRSS():
    ...
    if not os.path.exists(f'blackhole/rss/{request.args.get("id")}.xml'):
        return "rss file unavailable for this category at the moment"
    with open(f'blackhole/rss/{request.args.get("id")}.xml', "r") as rssFile:
        txt = re.sub(
...
def remoteTempTorrent():
    ...
    for torrentFile in os.listdir("blackhole/torrents/tmp/"):
        if os.stat(f"blackhole/torrents/tmp/{torrentFile}").st_mtime < now:
            os.remove(f"blackhole/torrents/tmp/{torrentFile}")
